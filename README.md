...
Shows: `ask_user`, `query`, `map_layer`, and the fixed answer layout (service date, result, caveats, how it was computed). The card in the video says it: *every answer shows its SQL, so it can be reproduced and verified.*
...
Not in a video: asking "is Route 1 running on time right now?" gets a refusal, since the feed is the schedule, not operations. See [Design for trust](#design-for-trust).

## Evaluation

Eight questions (five clear, three deliberately vague), each run three times per model configuration, checked at two levels: **data accuracy** (the key values appear in a successful query result, i.e. the table the planner sees) and **answer accuracy** (they also appear in the final answer text). Tool precision/recall compares the tools used against the tools a correct run needs. A control group runs the vague questions with `ask_user` removed. Method, golden set and what is traced: [docs/observability-evaluation.md](docs/observability-evaluation.md).
...
These are findings about building the agent, not about the MBTA schedule; the schedule findings are under each video above. The numbers are from the [Evaluation](#evaluation) section.
...
2. **Medium reasoning effort is the sweet spot, and the open-weight model is not there yet.** High effort costs more for the same accuracy; Gemini gets every table right but over-draws charts at 11× the cost; Llama 4 Maverick gets the table right about a quarter of the time. See the [comparison table](#evaluation).
...
Why these choices, and what I'd change at larger scale: [docs/tech-choices.md](docs/tech-choices.md)
